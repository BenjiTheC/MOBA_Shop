...
router.post('/:itemId/purchase', async (req, res) => {
    ...
    if (parseInt(user.virtualConcurrency) <= 0 || parseInt(user.virtualConcurrency) < parseInt(item.amount)) {
        return res.status(400).send('You don\'t have enough money!!!')
    }

    // charge user's money
    await userData.updateVirtualConcurrency(uid, user.virtualConcurrency - item.amount)
